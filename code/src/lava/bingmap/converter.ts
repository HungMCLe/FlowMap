...
export function fitOptions(bounds: IBound[], view: ISize): IViewOptions {
    ...
    let rectWidth = e - w;
    if (rectWidth < 0) rectWidth += 360;
    let width = helper.lon2x(rectWidth, 20);
    ...
    return { zoom: level, center: { latitude: centerLat, longitude: centerLon } };
}

export function anchorPixel(m: any, bound: IBound): IPoint {
    if (!m) return { x: 0, y: 0 };
    let level = m.getZoom(), { anchor: anch, margin } = bound;
    const point = m.latLngToContainerPoint([anch.latitude, anch.longitude]);
    let pix = { x: point.x, y: point.y } as IPoint;
...
export function area(data: ILocation[], level = 20): IArea {
    let areaResult = bound(data) as any as IArea;
    if (!areaResult) {
        return null;
    }
    let offsets = areaResult.offsets;
    let { longitude: alon, latitude: alat } = areaResult.anchor;
    ...
    areaResult.points = points;
    areaResult.scale = z => Math.pow(2, z - level);
    return areaResult;
}
...
namespace helper {
    ...
    export function loc(pixelX: number, pixelY: number, level: number): ILocation {
        var size = mapSize(level);
        var x = Math.min(pixelX, size - 1) / size - 0.5;
        var y = 0.5 - Math.min(pixelY, size - 1) / size;
